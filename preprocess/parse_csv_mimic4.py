...
class EHRParser:
    ...
    def calibrate_patient_by_admission(self):
        ...
        del_pids = []
        all_have_note_id = pickle.load(open('all_admission_id.pkl', 'rb'))
        for pid, admissions in self.patient_admission.items():
            for admission in admissions:
                ...
                if adm_id not in self.admission_codes:
                    break
                if adm_id not in all_have_note_id:
                    break
    ...
    def sample_patients(self, sample_num, seed):
        ...
        #pickle.dump(self.patient_admission, open('All_patient_admission.pkl', 'wb'))
        # all_have_note_id=pickle.load(open('all_admission_id.pkl','rb'))
        # del_pids = []
        # for i in list(self.patient_admission.keys()):
        #     if i not in all_have_note_id:
        #         del_pids.append(i)

        #print(len(self.patient_admission))
    ...
    def parse(self, sample_num=None, seed=6669):
        ...
        return self.patient_admission, self.admission_codes,self.users_data
    def parse2(self):
        self.parse_user()
...
class Mimic4Parser(EHRParser):
    ...
    def set_user(self):
        ...
        cols={self.pid_col:'subject_id',self.gender_col:'gender',self.age_col:'anchor_age',self.cluster_col:'CLUSTERS'}
        converter={
            'SUBJECT_ID': int,
            'GENDER':Mimic4Parser.gendertoId,
            "DOB":int,
