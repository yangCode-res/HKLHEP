...
def generate_code_code_adjacent(pids, patient_admission, admission_codes_encoded, code_num, threshold=0.01):
    ...
    #print(adj)
    return adj
def generate_catetocate_adjacent(pids,patient_admission,admission_codes_encoded,cate_num,threshold=0.1):
    np.set_printoptions(threshold=np.inf)
    print('generating cate to cate adjacent matrix ...')
    cate_dict=pickle.load(open('encode_icd9tocatecory.pkl','rb'))
    n = cate_num
    adj = np.zeros((n, n), dtype=int)
    for i, pid in enumerate(pids):
        print('\r\t%d / %d' % (i, len(pids)), end='')
        for admission in patient_admission[pid]:
            ...
            for row in range(len(codes) - 1):
                for col in range(row + 1, len(codes)):
                   # print(codes[row],codes[col])
                    c_i = cate_dict[codes[row]]
                    c_j = cate_dict[codes[col]]
                    adj[c_i, c_j] += 1
                    adj[c_j, c_i] += 1
    print('\r\t%d / %d' % (len(pids), len(pids)))
    #print(adj)
